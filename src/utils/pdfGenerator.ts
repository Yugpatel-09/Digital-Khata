...
export function generateCustomerStatementPDF(
  ...
  const tableData = sortedTxns.map((t, index) => {
    ...
    const balText = `${Math.abs(running).toLocaleString('en-IN')} (${running > 0 ? 'Dr' : running < 0 ? 'Cr' : '0'})`;
    ...
    return [
      ...
      t.paymentMode || 'Cash',
      isGave ? `${t.amount.toLocaleString('en-IN')}` : '-',
      !isGave ? `${t.amount.toLocaleString('en-IN')}` : '-',
      balText
    ];
  });
  ...
  // Top Accent Line (Emerald / Gold gradient look)
  ...
  doc.text(`Generated: ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })} at ${new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`, 14, 28);
